...
class WorkflowBackendTests(unittest.TestCase):
    ...
    def test_verification_false_positive_and_read_state(self) -> None:
        item = self.store.create_from_findings(job_id="job-2", work_area="A1工区", findings=[self.finding()])[0]
        verified = self.store.verify(item["id"], "安全员", True)
        self.assertEqual("待整改", verified["status"])
        self.assertEqual("安全员", verified["verified_by"])
        other = self.store.create_from_findings(job_id="job-3", work_area="A1工区", findings=[self.finding("H004")])[0]
    ...
    def test_rectification_review_major_confirmation_and_email_status(self) -> None:
        item = self.store.create_from_findings(job_id="job-4", work_area="A1工区", findings=[self.finding(severity="major")])[0]
    ...
    def test_public_workflow_routes_use_persistent_store(self) -> None:
        item = self.store.create_from_findings(job_id="job-api", work_area="A1工区", findings=[self.finding()])[0]
