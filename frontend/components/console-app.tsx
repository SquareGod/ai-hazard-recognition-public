  ...
  return <div className="page-stack">
    ...
    <section className="dispatch-layout">
      ...
      {current && <article className="surface notification-detail">
        ...
        <div className="dispatch-evidence"><span className="evidence-frame"><img className="evidence-blur" src={current.beforeImage} alt="" aria-hidden/><img src={current.beforeImage} alt="本次隐患证据"/></span><div><span>同一张图片识别到 {current.hazards.length} 项隐患</span>{current.hazards.map((hazard, index) => <button key={hazard.id} onClick={() => openHazard(hazard.id)}><b>{index + 1}. {hazard.title}</b><small><LevelBadge level={hazard.level}/><StatusBadge status={hazard.status}/></small><p>{hazard.evidence}</p></button>)}</div></div>
