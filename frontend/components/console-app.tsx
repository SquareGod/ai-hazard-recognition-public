...
function DispatchPage({ hazards, openHazard, flash }: { hazards: Hazard[]; openHazard: (id: string) => void; flash: (message: string) => void }) {
  ...
  const [evidenceZoom, setEvidenceZoom] = useState("");
  ...
  return <div className="page-stack">
    ...
    <section className="dispatch-layout">
      ...
      {current && <article className="surface notification-detail">
        ...
        <div className="dispatch-evidence">{evidenceZoom && <div className="modal-backdrop evidence-lightbox" onClick={() => setEvidenceZoom("")}><img className="evidence-full" src={evidenceZoom} alt="证据完整大图"/></div>}<button type="button" className="evidence-viewer" onClick={() => setEvidenceZoom(current.beforeImage)} title="点击查看完整大图"><img src={current.beforeImage} alt="本次隐患证据"/><span className="evidence-zoom-hint">点击查看完整大图</span></button><div><span>同一张图片识别到 {current.hazards.length} 项隐患</span>{current.hazards.map((hazard, index) => <button key={hazard.id} onClick={() => openHazard(hazard.id)}><b>{index + 1}. {hazard.title}</b><small><LevelBadge level={hazard.level}/><StatusBadge status={hazard.status}/></small><p>{hazard.evidence}</p></button>)}</div></div>
