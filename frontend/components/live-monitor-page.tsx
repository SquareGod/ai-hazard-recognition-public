  ...
  return <div className="monitor-page">
    ...
    <section className="monitor-wall" ref={wallRef}>
      ...
      {!loading && !error && primaryCamera && <article className="monitor-primary-card">
        ...
        {primary ? <ManagedStreamPlayer camera={{ id: primary.id, name: primary.name, webrtcUrl: primary.webrtc, hlsUrl: primary.hls }} backdropSrc={api.monitorSnapshotUrl?.(primary.id)} onEnlarge={() => setEnlargedId(primary.id)} /> : <div className="managed-video-stage monitor-switching-stage">{api.monitorSnapshotUrl && <img src={api.monitorSnapshotUrl(primaryCamera.id)} alt={`${primaryCamera.name}切换中画面`} />}<div className="player-state offline"><span>正在连接 {primaryCamera.name} 实时画面…</span></div></div>}
